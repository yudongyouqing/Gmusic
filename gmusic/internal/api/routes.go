...
func SetupRouter(db *gorm.DB) *gin.Engine {
	...
	if err != nil { fmt.Printf("播放器初始化失败: %v\n", err) }
...
func searchSongs(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		keyword := c.Query("q")
		if keyword == "" { c.JSON(http.StatusBadRequest, gin.H{"error": "搜索关键词不能为空"}); return }
...
func getSongByID(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if result.Error != nil { c.JSON(http.StatusNotFound, gin.H{"error": "歌曲不存在"}); return }
...
func addSong(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req struct{ FilePath string `json:"file_path" binding:"required"` }
		if err := c.ShouldBindJSON(&req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
		song, err := metadata.ExtractMetadata(req.FilePath)
		if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
		if err := storage.AddSong(db, song); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
...
func updateSong(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		id := c.Param("id")
		var song storage.Song
		if err := db.First(&song, id).Error; err != nil { c.JSON(http.StatusNotFound, gin.H{"error": "歌曲不存在"}); return }
...
func playHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if _, err := os.Stat(req.FilePath); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("文件不存在或不可读: %v", err)}); return }
		if err := audioPlayer.Play(req.FilePath); err != nil { fmt.Printf("播放失败: %v\n", err); c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
...
func seekHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		var req struct{ Position float64 `json:"position" binding:"required"` }
		if err := c.ShouldBindJSON(&req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
		if req.Position < 0 { req.Position = 0 }
		if err := audioPlayer.SeekTo(req.Position); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
		c.JSON(http.StatusOK, gin.H{"position": req.Position})
	}
}

func pauseHandler() gin.HandlerFunc { return func(c *gin.Context) { audioPlayer.Pause(); c.JSON(http.StatusOK, gin.H{"message": "已暂停"}) } }
func resumeHandler() gin.HandlerFunc { return func(c *gin.Context) { audioPlayer.Resume(); c.JSON(http.StatusOK, gin.H{"message": "已恢复"}) } }
func stopHandler() gin.HandlerFunc { return func(c *gin.Context) { audioPlayer.Stop(); c.JSON(http.StatusOK, gin.H{"message": "已停止"}) } }
func setVolumeHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		var req struct{ Volume float32 `json:"volume" binding:"required"` }
		if err := c.ShouldBindJSON(&req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
		audioPlayer.SetVolume(req.Volume); c.JSON(http.StatusOK, gin.H{"volume": req.Volume})
	}
}
func getPlayerStatus() gin.HandlerFunc {
	return func(c *gin.Context) { c.JSON(http.StatusOK, gin.H{"is_playing": audioPlayer.IsPlaying(), "position": audioPlayer.GetCurrentPosition(), "duration": audioPlayer.GetDuration()}) }
}
...
func refreshDurations(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := db.Find(&songs).Error; err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
		updated, skipped := 0, 0
		for i := range songs {
			if songs[i].Duration > 0 { skipped++; continue }
			sec := metadata.ComputeDurationSeconds(songs[i].FilePath)
			if sec > 0 { songs[i].Duration = sec; if err := db.Model(&songs[i]).Update("duration", sec).Error; err == nil { updated++ } }
		}
...
func getLyrics(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var song storage.Song
		if err := db.First(&song, songID).Error; err != nil { c.JSON(http.StatusNotFound, gin.H{"error": "歌曲不存在"}); return }
		lrcContent, err := metadata.ExtractLyrics(song.FilePath)
		if err != nil { c.JSON(http.StatusNotFound, gin.H{"error": "歌词文件不存在"}); return }
...
func scanDirectory(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req struct{ DirPath string `json:"dir_path" binding:"required"`; Workers int `json:"workers"` }
		if err := c.ShouldBindJSON(&req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
		if req.Workers == 0 { req.Workers = 4 }
		s := scanner.NewScanner(db)
		go func() {
			result, err := s.ScanDirectoryWithWorkers(req.DirPath, req.Workers)
			if err != nil { fmt.Printf("扫描错误: %v\n", err); return }
...
func getCover(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var song storage.Song
		if err := db.First(&song, songID).Error; err != nil { c.JSON(http.StatusNotFound, gin.H{"error": "歌曲不存在"}); return }
		if song.CoverURL == "" { c.JSON(http.StatusNotFound, gin.H{"error": "封面不存在"}); return }
		if _, err := os.Stat(song.CoverURL); err != nil { c.JSON(http.StatusNotFound, gin.H{"error": "封面文件不存在"}); return }
...
func playerWebSocket() gin.HandlerFunc {
	return func(c *gin.Context) {
		ws, err := upgrader.Upgrade(c.Writer, c.Request, nil); if err != nil { fmt.Printf("WebSocket 升级失败: %v\n", err); return }
		defer ws.Close()
		for {
			...
			if err := ws.WriteJSON(status); err != nil { break }
		}
	}
}
