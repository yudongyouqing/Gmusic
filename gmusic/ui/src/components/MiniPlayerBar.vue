    ...
    <div class="mini-bar__content">
      ...
      <div class="controls-wrap" @click.stop>
        <button class="btn small" :class="{active: store.playMode==='shuffle'}" title="播放顺序：随机/列表循环" @click="toggleMode">
          <svg v-if="store.playMode==='shuffle'" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            <!-- 上方路径：左上到右上，带箭头 -->
            <path d="M4 6h4l4 6h4" />
...
const tickerText = computed(() => {
  ...
  return lines[idx]?.text || `${s.title || '未知标题'} - ${s.artist || '未知歌手'}`
})
...
.ticker__inner { width: 100%; text-align: center; color:#222; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-size: var(--lyric-font-size, 14px); }
