...
const local = reactive({
  ...
  alpha: ui.alpha,
  blur: ui.blur,
  saturate: ui.saturate,
  tickerFontSize: ui.tickerFontSize,
  tickerOffsetX: ui.tickerOffsetX
})
...
function apply(){
  ...
  ui.tickerFontSize = local.tickerFontSize
...
function reset(){
  ...
  local.blur = 18
  local.saturate = 160
  local.tickerFontSize = 14
  local.tickerOffsetX = 0
  apply()
}
...
<style scoped>
.theme-pop{ width: 280px; padding: 14px; border-radius: 12px; background: var(--mica-surface); backdrop-filter: blur(var(--mica-blur)) saturate(var(--mica-saturate)); -webkit-backdrop-filter: blur(var(--mica-blur)) saturate(var(--mica-saturate)); border: 1px solid var(--mica-border); box-shadow: 0 20px 40px rgba(0,0,0,0.25); }
